...
## 6. CPU Architecture & Design Philosophy (Project 05)

Hack CPUは、命令のフェッチ、デコード、実行を行うコンピュータの心臓部である。
本章では、CPUの実装において直面する重要なハードウェア設計のパラダイムをまとめる。

### 6.1. データパスとコントロールパスの分離

CPUの内部回路は、大きく2つの役割に分かれている。

* **データパス (Data Path):**
  データが物理的に流れる経路（Mux, レジスタ, ALU）。
  **特徴:** 条件分岐（if文）で「止める」ことはせず、常に電気が流れ、計算が行われている。

* **コントロールパス (Control Path):**
  命令コード（Instruction）を解読し、データパスの各所にある「関所（Muxのselピンや、レジスタのloadピン）」を操作する経路。
  **特徴:** 常に計算されているデータの中から、「どれを採用し、どこに保存するか」を決定する。

### 6.2. 【重要考察】無意味な計算の許容（止めるコスト）

ソフトウェアの常識では、「A命令（`@100`など、計算を伴わない命令）の時は、ALUの計算処理をスキップする」のが最適化である。
しかし、ハードウェア（CPU回路）においては、**A命令の時でもALUは全力で無意味な計算を行い続けている。**

**理由:**

1. **回路は常に通電している:** ALUは組み合わせ回路であり、入力がある限り瞬時に結果を出し続ける。

2. **遮断コストの高さ:** ALUの計算を「止める（入力をゼロにする等）」ためには、ALUの前に新たな論理ゲート（スイッチ）を多数追加する必要があり、回路規模の増大と遅延（レイテンシ）の悪化を招く。

3. **結果を捨てるのが最良:** したがって、「ALUには勝手に計算させておき、コントロールパス（A命令時は `isCInstruction = 0` となる）によって、レジスタやメモリの `load` 信号を `0` にし、**デタラメな計算結果を書き込ませない（捨てる）**」という設計が、最も高速で安上がりなアーキテクチャとなる。

### 6.3. レジスタの役割と「時間」の概念（t と t+1）

CPU内には、ALUの出力を入力に戻す「ループ配線」（例: `A=A+1` のような自己書き換え）が無数に存在する。
これが電気的にショート（発振）しないのは、**レジスタ（順序回路）が「時間の関所」として機能しているから**である。

* **時刻** $t$ **(組み合わせ回路の伝播):**
  クロックサイクルの中において、レジスタは「現在の値（時刻 $t$ の値、例: `outD`）」を頑なに出力し続ける。
  この値がALU等（組み合わせ回路）を光の速さで駆け巡り、新しい計算結果（例: `aluOut`）となってレジスタの入力ピン (`in`) に到達し、待機する。

* **時刻** $t+1$ **(順序回路の更新):**
  クロックが「カチッ」と鳴る（エッジトリガ）瞬間、関所が開き、入力ピンで待機していた新しい値がレジスタ内部に取り込まれ、新たな出力（時刻 $t+1$ の値）となる。

レジスタ（フリップフロップ）が存在しなければ、コンピュータに「状態」や「手順（プログラム）」は存在し得ない。レジスタが物理的な電気信号の流れを「クロック」という単位で区切ることで、初めて「時刻 $t$ の状態から、時刻 $t+1$ の状態を計算する」という論理的なステップ実行が可能になっている。

## 7. Computer Architecture

Hack Computerは、これまでに構築したすべてのハードウェアコンポーネントを統合し、プログラムを自律的に実行できる1台の完全なシステムである。
ノイマン型アーキテクチャ（より厳密には、命令メモリとデータメモリが物理的に分かれているハーバード・アーキテクチャ）に基づいている。

### 7.1. コンポーネントの統合

システムは以下の3つの主要コンポーネントから構成され、それぞれがバス（データ線）によって相互に接続される。

1. **ROM32K (命令メモリ):**
   * プログラム（機械語の命令列）が格納される読み取り専用メモリ。
   * CPUから `pc` (プログラムカウンタ) を受け取り、そのアドレスにある `instruction` (命令) をCPUに渡す。

2. **CPU (中央演算処理装置):**
   * ROMから受け取った `instruction` を解読し、ALUで計算を実行する。
   * 計算結果に基づいて、データメモリ(RAM)への読み書き (`inM`, `outM`, `writeM`, `addressM`) を制御する。
   * 次に実行する命令のアドレス (`pc`) を決定し、ROMに出力する。

3. **Memory (データメモリ & I/O):**
   * RAM領域、Screen領域、Keyboard領域を統合した記憶装置。
   * CPUからデータ (`outM`), 書き込みフラグ (`writeM`), アドレス (`addressM`) を受け取り、読み書きを行う。
   * 現在のアドレスにあるデータ (`out`) をCPUの `inM` へ返す。

### 7.2. 統合配線仕様 (The Tripartite Loop)

3つの巨大なブロックは、以下のような「3つのループ」を描くように結線されている。このシンプルな結線こそが、コンピュータが自律的に動き続ける仕組みそのものである。

```hdl
    // 1. 命令フェッチのループ (CPU -> ROM -> CPU)
